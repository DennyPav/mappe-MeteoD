...
# ==================== PALETTE COLORI (INVARIATE) ====================
# ... (Mantengo le tue definizioni colori per brevità, sono corrette) ...
...
colors_p_cum = ["#ffffff", "#8fd3ff", "#00a6ff", "#0055ff", "#0000aa", "#32cd32", "#008000", "#ffff00", "#ffcc00", "#ff9900", "#ff4500", "#ff0000", "#b30000", "#ff1493", "#ff00ff", "#9400d3", "#4b0082", "#dadada", "#909090", "#505050", "#000000"]
...
colors_snow = ["#ffffff","#a1d99b","#74c476","#41ab5d","#e0e0e0","#c0c0c0","#a0a0a0","#fde0dd","#fa9fb5","#dd3497","#7a0177","#49006a"]
...
colors_snow_cum = ["#e5f5e0", "#c7e9c0", "#a1d99b", "#74c476", "#41ab5d", "#e0e0e0", "#c0c0c0", "#a0a0a0", "#fde0dd", "#fcc5c0", "#fa9fb5", "#f768a1", "#dd3497", "#ae017e", "#7a0177", "#49006a"]
...
colors_cape = ["#ffffff", "#0272fc", "#02bafc", "#02fcf2", "#05b2a5", "#02fc35", "#c4fc02", "#fc9302", "#fc1302", "#b2097c", "#ff00ff", "#7f02fc"]
...
colors_zt = ["#ffffff", "#8221ed", "#5300bb", "#1d09a8", "#0e2dac", "#1660a1", "#2489a5", "#2c9b95", "#17d4ce", "#00d65d", "#3ac303", "#aaed00", "#f8e300", "#efa700", "#e26901", "#d22c00", "#c40111", "#aa0259", "#a70086", "#f3129e", "#ed70aa", "#f6c1d3"]
boundaries_zt = [0, 250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 2750, 3000, 3250, 3500, 3750, 4000, 4250, 4500, 4750, 5000, 6000]
...
# Datetime object per calcoli
run_datetime_obj = datetime.strptime(f"{run_date}{run_hour}", "%Y%m%d%H").replace(tzinfo=timezone.utc)

# DATA RUN PER JSON (Formato: YYYYMMDD_HHmm)
JSON_RUN_DATE = run_datetime_obj.strftime("%Y%m%d_%H%M")
generated_files = []  # <--- LISTA PER IL JSON
...
dl_list = [("T_2M", None), ("TD_2M", None), ("TOT_PREC", None), ("PMSL", None),
           ...
           ("OMEGA", "isobaricInhPa-700"), ("FI", "isobaricInhPa-500"), ("U", "isobaricInhPa-700"), ("V", "isobaricInhPa-700"), ("VMAX_10M", None)]

print("\n⬇️ Download...", flush=True)
for v, f in dl_list: download_icon(v, f)

# ==================== UTILS CARICAMENTO (INVARIATO) ====================
...
print("🗺️ Caricamento capoluoghi...", flush=True)
capoluoghi_gdf = None
if os.path.exists(CAPOLUOGHI_GEOJSON_PATH):
    try:
        # Carica e converte in WGS84 per sicurezza
        capoluoghi_gdf = gpd.read_file(CAPOLUOGHI_GEOJSON_PATH).to_crs(epsg=4326)
        print("✅ Province GeoJSON OK!", flush=True)
    except Exception as e:
        print(f"⚠️ Errore capoluoghi GeoJSON: {e}", flush=True)
...
def setup_map():
    ...
    # 3. === AGGIUNTA CAPOLUOGHI (PALLINI) ===
    if capoluoghi_gdf is not None:
        # Assumiamo che il GeoJSON contenga Point. Se contiene poligoni, usa .centroid
        ax.scatter(
            capoluoghi_gdf.geometry.x, 
            capoluoghi_gdf.geometry.y, 
            c='red', 
            s=2,     # Dimensione pallino
            transform=ccrs.PlateCarree(), 
            zorder=35
...
def save_plot(name):
    if name.endswith(".png"): name = name.replace(".png", ".webp")
    plt.savefig(name, dpi=120, bbox_inches="tight", pil_kwargs={'quality': 65})
    plt.close()
...
def finalize_plot(fig, ax, cf, run_dt, valid_dt, title_bold, title_normal, cbar_label, explicit_ticks=None):
    kw_args = {}
    if explicit_ticks is not None: kw_args['ticks'] = explicit_ticks
    ...
    if isinstance(valid_dt, np.datetime64): valid_dt = pd.to_datetime(valid_dt).replace(tzinfo=timezone.utc)
    elif isinstance(valid_dt, pd.Timestamp) and valid_dt.tz is None: valid_dt = valid_dt.replace(tzinfo=timezone.utc)
...
for idx, t_val in enumerate(times):
    ...
    # Calcolo differenza ore per il campo "step" del JSON
    diff_hours = int((valid_dt_obj - run_datetime_obj).total_seconds() / 3600)
    
    t2 = t2m.sel(time=t_val, method='nearest')
    td = d2m.sel(time=t_val, method='nearest')
    msl_curr = msl.sel(time=t_val, method='nearest')
    
    if "latitude" in msl_curr.coords:
        msl_s = xr.DataArray(gaussian_filter(msl_curr, 4.0), coords=msl_curr.coords, dims=msl_curr.dims)
    else:
        msl_s = msl_curr
    
    try: prec = tp.sel(time=t_val, method='nearest').fillna(0)
    except: prec = xr.zeros_like(t2)
    ...
    # SALVATAGGIO IMMAGINI E AGGIORNAMENTO LISTA JSON
    
    fname = f"T2M_{step_str}.webp"
    fig, ax = setup_map()
    ...
    generated_files.append({"name": fname, "step": diff_hours})

    fname = f"PREC_{step_str}.webp"
    fig, ax = setup_map()
    ...
    fname = f"SNOWPACK_{step_str}.webp"
...
# Step convenzionale per i totali (es. ultima ora disponibile)
total_step = int((last_time_dt - run_datetime_obj).total_seconds() / 3600)

fname = "PREC_CUM_TOTALE_RUN.webp"
...
for idx, t_day in enumerate(daily_prec.time.values):
    ...
    fname = f"PREC_CUM_DAY_{day_idx_str}.webp"
...
for idx, day in enumerate(days):
    ...
    labels = ax.clabel(cs, inline=True, fontsize=7, fmt='%d')
    for lbl in labels:
        lbl.set_rotation(0)
...
json_path = os.path.join(OUTDIR, "status_icon.json")
...
def upload_to_r2():
    ...
    try:
        ...
        # Includiamo anche il file .json
        files = [f for f in os.listdir(OUTDIR) if os.path.isfile(os.path.join(OUTDIR, f)) and f.endswith(('.webp', '.txt', '.json'))]
        ...
        for filename in files:
            ...
            # GESTIONE CACHE: No-cache per JSON, normale per immagini
